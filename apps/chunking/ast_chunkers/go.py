...
class GoASTChunker:
    ...
    def _parse_interface_block(self, interface_node, source: bytes, all_comments: List, package_name: str) -> GoCodeBlock:
        ...
        def extract_blocks(node):
            ...
            try:
                if node.type == "package_clause":
                    ...
                    func_block = self._parse_function_block(node, source, all_comments, package_name)
                    blocks.append(func_block)
                
                elif node.type == "method_declaration":
                    method_block = self._parse_method_block(node, source, all_comments, package_name)
                        ...
                        if child.type == "type_spec":
                            for spec_child in child.children:
                                if spec_child.type == "struct_type":
                                    struct_block = self._parse_struct_block(node, source, all_comments, package_name)
                                    blocks.append(struct_block)
                                elif spec_child.type == "interface_type":
                                    interface_block = self._parse_interface_block(node, source, all_comments, package_name)
            ...
            for line in lines:
                ...
                if current_size + line_size > self.max_chunk_size and current_chunk_lines:
                    ...
                    chunk_block = GoCodeBlock(
                        ...
                        comments=block.comments if chunk_num == 1 else [],  # Only first chunk gets comments
...
def _fallback_chunk_go_code(source_code: str, max_chunk_size: int) -> List[Dict]:
    ...
    for i, line in enumerate(lines, 1):
        ...
        if (in_function or in_struct or in_interface) and brace_depth == 0 and stripped.endswith('}'):
        ...
        if should_end_chunk and current_chunk:
            chunk_text = '\n'.join(current_chunk)
            chunk_type = 'function' if in_function else 'struct' if in_struct else 'interface' if in_interface else 'code_block'
