# Create all 8 PRs to UPSTREAM repo (yichuan-w/LEANN)
# Uses fork:branch format so PRs pull from raoabinav/LEANN
# Run after: gh auth login

$upstreamRepo = "yichuan-w/LEANN"
$forkUser = "raoabinav"
$base = "main"

# Same PR data as create-prs.ps1
$prs = @(
    @{ head = "abinav/issue-177-cold-start-LONFb"; title = "Fix cold-start latency + ZMQ reliability"; body = @'
Closes #177, #182
...
'@ },
    @{ head = "abinav/issue-233-hybrid-search-LONFb"; title = "feat: add hybrid search via BM25 (SQLite FTS5) + score fusion"; body = @'
Closes #233
...
'@ },
    @{ head = "abinav/issue-141-reindex-LONFb"; title = "Add leann reindex CLI command with file manifest tracking"; body = @'
Closes #141
...
'@ },
    @{ head = "abinav/issue-158-ocr-LONFb"; title = "Add --enable-ocr flag for scanned PDF support"; body = @'
Closes #158
...
'@ },
    @{ head = "abinav/issue-217-llamaindex-LONFb"; title = "feat: add LlamaIndex integration with hybrid retriever support"; body = @'
Closes #217
...
'@ },
    @{ head = "abinav/issue-166-warmup-LONFb"; title = "feat: add persistent embedding daemon to eliminate cold-start latency"; body = @'
Closes #166
...
'@ },
    @{ head = "abinav/issue-96-obsidian-LONFb"; title = "feat: add Obsidian vault support with wikilink/backlink awareness"; body = @'
Closes #96
...
'@ },
    @{ head = "abinav/issue-47-local-cursor-LONFb"; title = "feat: add local Cursor proxy - local LLM + LEANN code retrieval"; body = @'
Closes #47
...
Write-Host "Creating PRs to UPSTREAM: $upstreamRepo" -ForegroundColor Cyan
Write-Host "From fork: $forkUser (head = $forkUser`:branch)" -ForegroundColor Cyan
Write-Host ""
...
foreach ($pr in $prs) {
    $head = "$forkUser`:$($pr.head)"
    Write-Host "Creating PR: $($pr.title)" -ForegroundColor Cyan
    $bodyFile = [System.IO.Path]::GetTempFileName()
    $pr.body | Out-File -FilePath $bodyFile -Encoding utf8
    gh pr create --repo $upstreamRepo --base $base --head $head --title $pr.title --body-file $bodyFile
