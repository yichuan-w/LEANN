...
def _openclaw_agent(message: str, timeout: int = 300) -> dict:
    ...
    result = subprocess.run(
        [
            "docker", "exec", DOCKER_CONTAINER, OPENCLAW_BIN, "agent",
            "--agent", "main",
            "-m", message,
            "--json",
            "--timeout", str(timeout),
...
def _leann_cmd(args: list[str], cwd: str | None = None, timeout: int = 60) -> subprocess.CompletedProcess[str]:
...
def openclaw_ready():
    ...
    if not _docker_running():
        pytest.skip(f"Docker container '{DOCKER_CONTAINER}' not running")  # type: ignore[call-overload]
...
class TestOpenClawMemoryFormation:
    ...
    def test_memory_md_content(self, openclaw_memory):
        ...
        if not memory_md.exists():
            pytest.skip("MEMORY.md not created")  # type: ignore[call-overload]
