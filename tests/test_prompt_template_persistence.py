...
def _stub_native_hnsw(monkeypatch):
    """Stub native HNSW graph build/load.

    Real HNSWBuilder.build with mocked 3-D embeddings aborts the Linux CI
    pytest process with exit code 127 (no Python traceback). These tests only
    need meta.json / embedding_options wiring and searcher query-embedding
    paths — not a real FAISS graph — so stub the native surface.
    """
    try:
    ...
    def fake_search(self, query, top_k, **kwargs):
        return {
            "labels": [[0] * top_k for _ in range(len(np.atleast_2d(query)))],
            "distances": np.zeros((len(np.atleast_2d(query)), top_k), dtype=np.float32),
        }
