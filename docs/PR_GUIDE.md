# Full PR Guide for `raoabinav/LEANN`

**Base branch for all PRs: `master`**

All 8 branches are independent — they each branched off `master` and don't depend on each other. You can open all 8 PRs simultaneously.

---

## PR 1: Fix cold-start latency + ZMQ reliability
**Branch:** `abinav/issue-177-cold-start-LONFb` → `master`  
**Closes:** #177, #182  
**Files:** `searcher_base.py`, `embedding_server_manager.py`, `tests/test_cold_start.py` (+512, -50)
...
## PR 2: Hybrid search (BM25 + vector score fusion)
**Branch:** `abinav/issue-233-hybrid-search-LONFb` → `master`  
**Closes:** #233  
**Files:** `api.py`, `bm25.py` (new), `hybrid.py` (new), `tests/test_hybrid_search.py` (+696, -1)
...
## PR 3: Reindex CLI with delta detection
**Branch:** `abinav/issue-141-reindex-LONFb` → `master`  
**Closes:** #141  
**Files:** `api.py`, `cli.py`, `tests/test_reindex.py` (+365)
...
## PR 4: OCR support for scanned PDFs
**Branch:** `abinav/issue-158-ocr-LONFb` → `master`  
**Closes:** #158  
**Files:** `cli.py`, `pyproject.toml`, `apps/ocr_rag.py` (new), `tests/test_ocr.py` (+322, -4)
...
## PR 5: LlamaIndex integration
**Branch:** `abinav/issue-217-llamaindex-LONFb` → `master`  
**Closes:** #217  
**Files:** `integrations/__init__.py` (new), `integrations/llamaindex.py` (new), `tests/test_llamaindex_integration.py` (+402)
...
## PR 6: Persistent embedding daemon
**Branch:** `abinav/issue-166-warmup-LONFb` → `master`  
**Closes:** #166  
**Files:** `cli.py`, `embedding_daemon.py` (new), `embedding_server_manager.py`, `tests/test_warmup_daemon.py` (+841)
...
## PR 7: Obsidian vault support
**Branch:** `abinav/issue-96-obsidian-LONFb` → `master`  
**Closes:** #96  
**Files:** `cli.py`, `obsidian.py` (new), `tests/test_obsidian.py` (+629)
...
## PR 8: Local Cursor proxy
**Branch:** `abinav/issue-47-local-cursor-LONFb` → `master`  
**Closes:** #47  
**Files:** `cli.py`, `local_cursor.py` (new), `tests/test_local_cursor.py` (+648)
...
1. **#177 cold-start fix** (smallest, pure bugfix, 2 commits) — low risk, merge first
2. **#233 hybrid search** — foundational feature, other things build on it
3. **#141 reindex CLI** — useful standalone
4. **#158 OCR** — useful standalone
5. **#217 LlamaIndex** — depends on hybrid search existing if you want the hybrid retriever to work
6. **#166 warmup daemon** — alternative approach to cold-start, complements #177
7. **#96 Obsidian** — standalone
8. **#47 Cursor proxy** — standalone
...
The script targets `master` as the base branch and uses the full descriptions above for each PR body.
