...
class CodeSymbolExtractor:
    ...
    Extracts:
    - CamelCase identifiers (e.g., getUserData -> get, user, data, getUserData)
    - snake_case identifiers (e.g., get_user_data -> get, user, data, get_user_data)
    - CONSTANT_CASE (e.g., API_KEY -> api, key, API_KEY, api_key)
    - File path components (e.g., /src/models/user.py -> user, models, src, user.py)
    - Import statements
    """
    ...
    def extract_symbols(cls, text: str, metadata: Optional[dict[str, Any]] = None) -> list[str]:
        ...
        Args:
            ...
            metadata: Optional metadata containing file_path, file_name, etc.
            
        Returns:
            List of extracted symbols (lowercased for BM25 compatibility)
        """
        symbols = []
    ...
    def _split_identifier(cls, identifier: str) -> list[str]:
        ...
        Examples:
            ...
            get_user_data -> ['get', 'user', 'data', 'get_user_data']
...
class SymbolAwareBM25Scorer(BM25Scorer):
    ...
    Features:
    - Code-aware symbol extraction (camelCase, snake_case, CONSTANTS)
    - Metadata field extraction (file_path, file_name, imports)
    - Exact match boosting for precise identifier queries
    """
    ...
    def fit(self, documents: list[dict[str, Any]]):
        """
        Build BM25 statistics with code-aware symbol extraction.
        """
    ...
    def _compute_exact_match_boost(self, query: str, doc_id: str) -> float:
        ...
        This gives a significant boost when:
        - The query exactly matches a function/class name in the document
        - The query matches a file name or path component
        """
        boost = 0.0
...
class LeannSearcher:
    ...
    def _init_bm25(self) -> None:
        """Initialize BM25 scorer with code symbol awareness"""
        self.bm25_scorer = SymbolAwareBM25Scorer()
        passages = []
